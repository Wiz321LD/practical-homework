...
CREATE TABLE auditorium(
    ...
    floor VARCHAR,
    main_group_number INT REFERENCES university_group (number) UNIQUE
);


CREATE TABLE first_floor_auditorium(
    room_number INT PRIMARY KEY,
    is_storage_present BOOLEAN,
    FOREIGN KEY (room_number) REFERENCES auditorium (room_number)
);


CREATE TABLE second_floor_auditorium(
    room_number INT PRIMARY KEY,
    type VARCHAR,
    FOREIGN KEY (room_number) REFERENCES auditorium (room_number)
);
...
DROP TABLE auditorium, first_floor_auditorium, second_floor_auditorium;


TRUNCATE TABLE auditorium, first_floor_auditorium, second_floor_auditorium
    RESTART IDENTITY;
...
EXPLAIN ANALYZE SELECT stud.* FROM student AS stud
    LEFT OUTER JOIN student_teacher st ON stud.student_id = st.student_id_fk
    LEFT OUTER JOIN teacher t ON st.teacher_id_fk = t.teacher_id
    WHERE stud.grade > 2000
    LIMIT 5;



CREATE INDEX student_grade_btree_index ON student(grade);